...
(mostly empty inside) with a solid top deck, so it prints fast and light while
the top reads as a clean surface with only the holes showing. Design history:
...
- Construction: hollow shell with a 3 mm solid top deck, 3 mm outer wall + floor,
  2.4 mm socket walls (~315 cm³ of material vs ~680 cm³ if printed solid)
...
- Supports: none — the sockets, divider, and an internal rib hold the top deck
  up, so it only bridges short (≤ ~10 mm) hidden spans
