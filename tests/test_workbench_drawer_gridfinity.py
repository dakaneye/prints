"""Smoke test for the workbench-drawer Gridfinity project.
...
PROJECT_3D = REPO_ROOT / "projects" / "workbench-drawer-gridfinity" / "3d"
...
def test_build_all_produces_valid_stls():
    ...
    # bins.py is empty until inventory is mapped, so only the baseplate
    # is expected. Expand this floor when BINS gets populated.
    assert len(stls) >= 1, f"Expected at least 1 STL (baseplate), got {len(stls)}"
    ...
    for stl in stls:
        ...
        w, d, h = mesh.extents
        # Footprint: smallest 1x1 cell ~42 mm; largest 6x6 baseplate ~252 mm.
